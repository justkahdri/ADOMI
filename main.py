...
# TODO - Pendientes
# Funcionalidades opecionales en collab del proycto final
...
def reportar(senal, lapsos, fr_cardiaca, estado):
    ...
    if not os.path.isdir('./exports'):
        ...
        while sobreescribir.lower() not in ['si', 'no', 's', 'n']:
            print('El archivo resultados.txt ya existe')
            sobreescribir = input('Desea sobreescribirlo? [S̲í/N̲o]: ')
        if sobreescribir.lower() in 'no':
...
def determinar_actividad(fr_cardiaca, edad, sexo, peso):
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            sexo_paciente = input("Ingrese sexo del paciente [M̲̲asculino/F̲emenino]: ").upper()
